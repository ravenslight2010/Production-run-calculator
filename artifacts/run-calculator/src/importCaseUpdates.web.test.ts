...
// ---------------------------------------------------------------------------
// Web ⇄ mobile parity — the two hand-maintained copies must produce identical
// offers for equivalent inputs, and equivalent warning text for flagged ones.
// ---------------------------------------------------------------------------
...
interface MobileModule {
  ...
  __confirmCalls: { message: string; onConfirm: () => void; onCancel: () => void }[];
}
...
async function loadStrippedModule(file: string): Promise<MobileModule> {
  ...
  const out = path.join(
    os.tmpdir(),
    `importCaseUpdates.parity.${process.pid}.${Date.now()}.mjs`,
  );
...
describe("web ⇄ mobile parity", () => {
  /** Feed the SAME logical scenario to both builders. Web reads casesNeeded +
   *  skid counters from a values store; mobile takes casesNeeded/casesMade on
   *  the run. Offers must come out identical. */
  function bothOffers(cases: {
    id: string;
    rowCases: number;
    current: number;
    made?: number;
    inProgress?: boolean;
  }[]): { web: CaseUpdateOffer[]; mob: CaseUpdateOffer[] } {
    const store: Record<string, RunCaseFields> = {};
    const webMatches: SkippedRowMatch[] = [];
    const mobMatches: {
      row: { brand: string; flavor: string; casesNeeded: number };
      run: MobileRanRunInfo;
    }[] = [];
    for (const c of cases) {
      const inProgress = c.inProgress ?? true;
      store[c.id] = valuesMade(c.current, c.made);
      webMatches.push({
        row: { brand: "Acme", flavor: "Pepperoni", casesPlanned: c.rowCases },
        run: { id: c.id, inProgress },
      });
      mobMatches.push({
        row: { brand: "Acme", flavor: "Pepperoni", casesNeeded: c.rowCases },
        run: {
          brand: "Acme",
          flavor: "Pepperoni",
          id: c.id,
          startedAt: 1_000,
          ...(inProgress ? {} : { endedAt: 2_000 }),
          casesNeeded: c.current,
          ...(c.made != null ? { casesMade: c.made } : {}),
        },
      });
    }
    return {
      web: buildCaseUpdateOffers(webMatches, (id) => store[id] ?? {}),
      mob: mobile.buildCaseUpdateOffers(mobMatches),
    };
  }

  it("produces identical offers across flagged / raised / equal / finished / invalid cases", () => {
    const { web, mob } = bothOffers([
      { id: "lowered-below-made", rowCases: 60, current: 100, made: 80 },
      { id: "lowered-to-made", rowCases: 80, current: 100, made: 80 },
      { id: "raised", rowCases: 150, current: 100, made: 80 },
      { id: "no-progress", rowCases: 60, current: 100 },
      { id: "finished", rowCases: 60, current: 100, made: 80, inProgress: false },
      { id: "unchanged", rowCases: 100, current: 100, made: 80 },
      { id: "bad-count", rowCases: 0, current: 100, made: 80 },
      { id: "fractional", rowCases: 80, current: 100, made: 80.6 },
    ]);
    expect(web).toEqual(mob);
    // And sanity-check the shared outcome, not just agreement:
    expect(web.map((o) => o.runId)).toEqual([
      "lowered-below-made",
      "lowered-to-made",
      "raised",
      "no-progress",
    ...
    expect(web[0].madeAlready).toBe(80);
    expect(web[1].madeAlready).toBeUndefined();
    expect(web[4].madeAlready).toBe(81);
  });

  it("flagged warning text carries the same facts on both platforms", () => {
    const { web, mob } = bothOffers([
      { id: "a", rowCases: 60, current: 100, made: 80 },
    ]);
    expect(web).toEqual(mob);
    const offer = web[0];

    // Web renders the warning as a dialog line…
    const webLine = caseUpdateWarningLine(offer);
    expect(webLine).not.toBeNull();

    // …mobile folds it into the chained confirm prompt.
    mobile.__confirmCalls.length = 0;
    mobile.promptCaseUpdates(mob, "", () => {});
    expect(mobile.__confirmCalls).toHaveLength(1);
    const mobMessage = mobile.__confirmCalls[0].message;

    // Same facts, same phrasing (mobile shouts BELOW; web sentence-cases it).
    for (const text of [webLine as string, mobMessage]) {
      const lower = text.toLowerCase();
      expect(lower).toContain("already made 80");
      expect(lower).toContain("the new target of 60 is below that");
      expect(lower).toContain("over target");
    }
  });

  it("unflagged offers carry no warning on either platform", () => {
    const { web, mob } = bothOffers([
      { id: "b", rowCases: 150, current: 100, made: 80 },
    ]);
    expect(web).toEqual(mob);
    expect(caseUpdateWarningLine(web[0])).toBeNull();

    mobile.__confirmCalls.length = 0;
    mobile.promptCaseUpdates(mob, "", () => {});
    const msg = mobile.__confirmCalls[0].message.toLowerCase();
    expect(msg).not.toContain("already made");
