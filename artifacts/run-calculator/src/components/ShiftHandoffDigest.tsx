...
import { Clipboard, Download, ExternalLink, Loader2, Lock, RefreshCw, ClipboardCheck } from "lucide-react";
...
const severityClass: Record<HandoffSeverity, string> = {
  urgent: "bg-red-500/15 text-red-400", high: "bg-red-500/10 text-red-400",
  medium: "bg-amber-500/15 text-amber-500", low: "bg-sky-500/15 text-sky-400", info: "bg-muted text-muted-foreground",
};
const sourceLabels: Record<HandoffItem["source"], string> = {
  incidents: "Incidents", quality: "Quality history", inventory: "Inventory", sync: "Sync diagnostics", "data-health": "Data health",
};
...
function handoffText(digest: ShiftHandoffDigest): string {
  ...
  for (const source of Object.keys(sourceLabels) as HandoffItem["source"][]) {
    const sourceState = digest.sources[source];
    lines.push(`${sourceLabels[source].toUpperCase()} — ${sourceState.availability === "available" ? `${sourceState.itemCount} item(s)` : "UNAVAILABLE"}`);
    if (sourceState.note) lines.push(`  ${sourceState.note}`);
    for (const item of digest.items.filter((entry) => entry.source === source)) {
      lines.push(`  [${item.severity.toUpperCase()} / ${item.status}] ${item.title}`);
...
export default function ShiftHandoffDigest({ onOpenSource }: { onOpenSource?: (source: HandoffItem["source"]) => void }) {
  ...
  return (
    <Card data-testid="shift-handoff-digest">
      <CardHeader className="pb-2"><div className="flex items-start justify-between gap-2"><div><CardTitle className="text-base flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-primary" /> Daily shift handoff</CardTitle><p className="text-xs text-muted-foreground mt-1">One scoped summary; source workflows remain the system of record.</p></div></div></CardHeader>
      <CardContent className="space-y-3">
        ...
        {digest && <div className="space-y-3 border-t border-border/60 pt-3">
          <div className="flex flex-wrap gap-1.5 text-[10px] font-semibold">
            {(["urgent", "high", "medium", "low", "info"] as HandoffSeverity[]).map((severity) => <span key={severity} className={`rounded px-2 py-0.5 ${severityClass[severity]}`}>{severity}: {digest.items.filter((item) => item.severity === severity).length}</span>)}
          </div>
          {digest.items.length === 0 ? <p className="rounded border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-500">No unresolved handoff items for this date.</p> : (
            <div className="space-y-2">
              {digest.items.map((item) => <div key={item.id} className="rounded border border-border bg-background/50 p-3">
