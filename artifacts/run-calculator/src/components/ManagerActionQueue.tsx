...
import { ClipboardCheck, ExternalLink, Loader2, Lock, RefreshCw } from "lucide-react";
...
export default function ManagerActionQueue({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  return <Card data-testid="manager-action-queue">
    <CardHeader className="pb-3">
      ...
      <p className="text-xs text-muted-foreground">One prioritized view of unresolved work. Source workflows remain the system of record.</p>
    </CardHeader>
    <CardContent className="space-y-3">
      <div className="flex flex-wrap gap-2">
        <select aria-label="Filter action status" className="rounded border border-border bg-background px-2 py-1.5 text-xs" value={filter} onChange={(e) => setFilter(e.target.value)}>
          <option value="open">Open ({query.data?.counts.open ?? 0})</option><option value="in_progress">In progress ({query.data?.counts.in_progress ?? 0})</option><option value="deferred">Deferred ({query.data?.counts.deferred ?? 0})</option><option value="resolved">Resolved ({query.data?.counts.resolved ?? 0})</option><option value="all">All</option>
        </select>
        <select aria-label="Filter action category" className="rounded border border-border bg-background px-2 py-1.5 text-xs" value={category} onChange={(e) => setCategory(e.target.value)}>
          <option value="all">All sources</option>{Object.entries(labels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
        </select>
      </div>
      {query.isLoading ? <div className="flex justify-center py-8"><Loader2 className="h-5 w-5 animate-spin" /></div> : query.isError ? <p className="text-sm text-destructive">Couldn’t load the manager action queue.</p> : items.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">{filter === "open" ? "No open actions. The facility is caught up." : "No actions match these filters."}</p> :
        <div className="space-y-2">{items.map((item) => <div key={item.id} className="rounded-md border border-border bg-background p-3">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-1.5"><span className="font-medium text-sm">{item.title}</span><span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${item.severity === "urgent" || item.severity === "error" ? "bg-destructive/15 text-destructive" : item.severity === "warning" ? "bg-amber-500/15 text-amber-600" : "bg-muted text-muted-foreground"}`}>{item.severity}</span><span className="rounded bg-muted px-1.5 py-0.5 text-[10px]">{labels[item.category]}</span></div><p className="mt-1 text-xs text-muted-foreground">{item.description} · {age(item.createdAt)}{item.assigneeName ? ` · ${item.assigneeName}` : " · Unassigned"}</p></div>
            <a className="inline-flex shrink-0 items-center gap-1 text-xs text-primary hover:underline" href={item.sourcePath} onClick={() => onNavigate?.(item.sourceType === "incident" ? "incidents" : item.sourceType === "sync" ? "summary" : "setup")}>Open source <ExternalLink className="h-3 w-3" /></a>
          </div>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <select aria-label={`Status for ${item.title}`} className="rounded border border-border bg-background px-2 py-1 text-xs" value={item.status} disabled={mutation.isPending} onChange={(e) => mutation.mutate({ item, input: { version: item.version, status: e.target.value as ActionItem["status"] } })}>
              <option value="open">Open</option><option value="in_progress">In progress</option><option value="deferred">Deferred</option><option value="resolved">Resolved</option>
            </select>
