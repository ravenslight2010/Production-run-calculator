...
The main config enumerates 161 cases and retains
...
and does not load the 161-case release reporter, even if
`PLAYWRIGHT_RELEASE_REPORT_PATH` is set. Its result is never release evidence.
The full `test:e2e` lane remains the only command that enforces exactly 161
