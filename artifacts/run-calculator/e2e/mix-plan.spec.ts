 ...
 *
 *   (k) Empty state is preserved after a full page reload when all runs are
 ...
 *
 *   (m) Pull For Mix per-component lbs on a regular mix card update live when
 *       "already made" is edited (Task #799).
 *       — Full lbs shown initially (amountAlreadyMade=0).
 *       — Scaled lbs shown after a partial already-made entry.
 *       — 0.00 shown after already-made >= totalLbs.
 *
 * Relevant files:
...
async function waitForTodayMixPlanCard(page: Page, today: string): Promise<Locator> {
  const card = page.locator(`[data-testid="mix-plan-${today}"]`);
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      await page.locator('[data-testid="tab-run"]')
        .waitFor({ state: "attached", timeout: 25_000 });
...
async function dbSeedProfile(
  ...
  const key = `${opts.brand.toLowerCase().trim()}__${opts.flavor.toLowerCase().trim()}`;
  await db.query(
    `INSERT INTO brand_profiles
       (key, scope, brand, flavor, values, crust_values, updated_at_ms, created_at)
     VALUES ($1, 'live', $2, $3, $4::jsonb, '{}'::jsonb, $5, NOW())
    ...
    [
      ...
      opts.flavor,
      JSON.stringify({
        pep1Type: opts.ingredient,
        pep1OzPerPizza: opts.runOz,
        pizzasPerCase: opts.pizzasPerCase,
      }),
      Date.now(),
    ],
  );
...
test.describe("Mix Plan — prep card suppression and ended-run removal", () => {
  ...
  test("prep mix card is absent when no active run lists its component ingredient", async ({
    ...
    try {
      ...
      // Other suites may leave unrelated factory-wide prep recipes in the
      // shared master-data pool. The invariant here is that this exact mix and
      // component are absent without a qualifying active run.
    } finally {
  ...
  test("mix card disappears from the plan after the run is marked ended", async ({
    ...
    try {
      ...
      await goToMixes(page);
      const todayCard = page.locator(`[data-testid="mix-plan-${today}"]`);
      await todayCard.waitFor({ state: "visible", timeout: 20_000 }).catch(async () => {
        await page.reload({ waitUntil: "domcontentloaded" });
        await page.locator('[data-testid="tab-run"]')
          .waitFor({ state: "attached", timeout: 25_000 });
  ...
  test("retried already-made amount survives a later Mix Plan reload", async ({
    ...
    const runId = `reg-retry-reload-run-${suffix}`;
    const today = todayStr();
    const alreadyMade = 50;
    const totalLbs = (2 * 100 * 8 / 16) * 1.15 + 20;
    const remainingLbs = totalLbs - alreadyMade;
    ...
    try {
      ...
      await goToMixes(page);
      const todayCard = await waitForTodayMixPlanCard(page, today);
      await expect(todayCard.getByText(mixName, { exact: false })).toBeVisible({ timeout: 5_000 });
      ...
      await expect.poll(async () => {
        ...
        return payload.dayState?.runs?.some((run) => run.id === runId) ?? false;
      ...
      await reloadedCard.waitFor({ state: "visible", timeout: 20_000 }).catch(async () => {
        await page.reload({ waitUntil: "domcontentloaded" });
        await page.locator('[data-testid="tab-run"]')
          .waitFor({ state: "attached", timeout: 25_000 });
        await goToMixes(page);
   ...
   * from the in-memory state path.
   *
   * Mechanism: liveRunsForMixes (home.tsx ~14462) re-derives its list from
   * dayState.runs on every render. loadDayState() on mount reads the persisted
   * day state from localStorage, which carries endedAt on every run. If the
   * filter regresses on the hydration path (e.g. endedAt is lost during
   * serialisation), the plan would wrongly re-appear after reload even though
   * it was empty in the same browser session.
   *
   * This test shares the same two-run stop sequence as test (j) and then adds
   * a full page reload before the final empty-state assertion.
   */

  /**
   * Test (l): Empty state is preserved after a full page reload when all runs
   * are ended — exercises the localStorage + React hydration path separately
   * from the in-memory state path.
   */
  test("empty state is preserved after a page reload when all runs are ended", async ({
    ...
    const mixId2 = `reload-ended-mix-b-${suffix}`;
    const mixName1 = `ReloadEndedMixA ${suffix}`;
    const mixName2 = `ReloadEndedMixB ${suffix}`;
    const component1 = `ReloadEndedCompA_${suffix}`;
    const component2 = `ReloadEndedCompB_${suffix}`;
    ...
    try {
      ...
      // endedAt must survive serialisation so liveRunsForMixes returns [] and
      // buildMixPlan returns [] after mount.
      await page.reload({ waitUntil: "domcontentloaded" });
      await page.locator('[data-testid="tab-run"]').waitFor({ state: "attached", timeout: 25_000 });
      // Dismiss onboarding if it reappears after reload (it should not, but be safe).
      await page.getByRole("button", { name: /^get.?started$/i })
        .waitFor({ state: "visible", timeout: 5_000 }).then((b) => b.click()).catch(() => {});
      await page.waitForTimeout(1_000);

      // Navigate to Mixes and assert empty state is still shown
      await goToMixes(page);
      await page.waitForTimeout(500);
  ...
  test("batch count on cold load uses persisted amountAlreadyMade from the server", async ({
    ...
    // Full-suite authenticated bootstrap can require one recovery reload after
    // a prior test's local state is replaced. Keep the complete assertion
    // sequence intact while allowing that deterministic recovery to finish.
    test.setTimeout(90_000);
    const suffix = uid();
    const username = `user_${suffix}`;
    ...
    const today = todayStr();
    const runId = `cold-batch-run-${suffix}`;
    ...
    try {
      ...
      // ── Step 2: seed the run before authenticated bootstrap ──────────────────
      await seedLiveRunBeforeAppLoad(page, {
        runId,
        brand,
        ingredient: component,
  ...
  /**
   * Test (n): Pull For Mix per-component lbs on a REGULAR (non-prep) mix card
   * update live when "already made" is edited. — Task #799.
   *
   * Regular mixes are brand/flavor-matched (not ingredient-matched like prep
   * mixes). Component pull lbs come from the mix's own perPizza value scaled to
   * the run's pizza count. The live formula in home.tsx ~14596:
   *
   *   pullLbs = m.totalLbs > 0 ? c.lbs * m.remainingLbs / m.totalLbs : 0
   *
   * where:
   *   c.lbs        = (perPizza / 16) × pizzas  — constant, doesn't change
   *   m.totalLbs   = c.lbs × 1.15 + 20         — constant, doesn't change
   ...
   * buildMixPlan re-runs → Pull For Mix re-renders.
   */
  test("Pull For Mix per-component lbs on a regular mix card update live when 'already made' is edited", async ({
    page,
  }) => {
    const suffix = uid();
    const username = `user_${suffix}`;
    ...
    try {
      ...
      // NOT the violet Ingredient Prep section).
      const alreadyMadeInput = todayCard.locator('input[type="number"]').first();
      await alreadyMadeInput.waitFor({ state: "visible", timeout: 5_000 });
      await alreadyMadeInput.click();
