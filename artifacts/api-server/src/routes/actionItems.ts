...
const severities = new Set(["info", "warning", "error", "urgent"]);
const categories = new Set(["incident", "import", "data-health", "sync", "production-rule", "report"]);

type Candidate = {
  dedupKey: string; category: string; severity: string; title: string;
  description: string; sourceType: string; sourceId: string; sourcePath: string;
};
...
async function candidates(): Promise<Candidate[]> {
  ...
  const [incidents, imports, conflicts, rules, health] = await Promise.all([
    db.select().from(incidentsTable).where(and(eq(incidentsTable.scope, scope), inArray(incidentsTable.workflowState, ["new", "assigned", "waiting"]))),
    db.select().from(importHistoryTable).where(and(eq(importHistoryTable.scope, scope), inArray(importHistoryTable.status, ["partial", "failed"]))),
    db.select().from(syncConflictLogsTable).where(and(eq(syncConflictLogsTable.scope, scope), sql`${syncConflictLogsTable.conflictCount} > 0`)),
    db.select().from(productionRulesTable).where(and(eq(productionRulesTable.scope, scope), eq(productionRulesTable.enabled, true))),
    dataHealthWorkspace(db),
  ]);
  const out: Candidate[] = [];
  for (const item of incidents) out.push({
    dedupKey: `incident:${item.id}`, category: "incident",
    ...
    description: clean(item.context && typeof item.context === "object" ? (item.context as Record<string, unknown>).description : "", 500) || item.screen,
    sourceType: "incident", sourceId: item.id, sourcePath: `#incidents/${encodeURIComponent(item.id)}`,
  });
  for (const item of imports) out.push({
    dedupKey: `import:${item.id}`, category: "import", severity: item.status === "failed" ? "error" : "warning",
  ...
  for (const item of conflicts) out.push({
    dedupKey: `sync:${item.id}`, category: "sync", severity: item.conflictCount > 5 ? "error" : "warning",
    title: "Sync conflict needs review", description: `${item.conflictCount} conflicting field${item.conflictCount === 1 ? "" : "s"} on ${item.date}`,
  ...
  for (const item of await candidates()) {
    await db.insert(actionItemsTable).values({ scope, ...item }).onConflictDoUpdate({
      ...
      set: {
        ...
        sourceId: item.sourceId, sourcePath: item.sourcePath,
        updatedAt: new Date(),
      },
    });
  }
...
router.get("/manager-action-queue", requireCapability("manage-staff"), async (req: Request, res: Response) => {
  try {
    ...
    res.json({ items: rows, counts: Object.fromEntries(["open", "in_progress", "deferred", "resolved"].map((status) => [status, rows.filter((row) => row.status === status).length])) });
  } catch (err) {
...
router.patch("/manager-action-queue/:id", requireCapability("manage-staff"), async (req: Request, res: Response) => {
  ...
  if (!Number.isInteger(id) || !Number.isInteger(version) || (status !== undefined && !statuses.has(status)) ||
    ...
    res.status(400).json({ error: "Invalid action item update" }); return;
  }
  try {
    ...
    if (body.assigneeId !== undefined) {
      assigneeId = body.assigneeId === "me" ? req.userId! : body.assigneeId === null || body.assigneeId === "" ? null : clean(body.assigneeId, 160);
      if (assigneeId) {
        const staff = await getStaffMember(assigneeId);
        if (staff.sandbox || !staff.name) { res.status(400).json({ error: "Assignee is not eligible" }); return; }
    ...
    const updated = await db.update(actionItemsTable).set(patch).where(and(
      eq(actionItemsTable.id, id), eq(actionItemsTable.scope, scope), eq(actionItemsTable.version, version),
    )).returning();
    if (!updated[0]) { res.status(409).json({ error: "This action item changed; refresh and try again." }); return; }
    await db.insert(auditLogsTable).values({
      scope, actor: actor.name ?? req.userId!, action: "manager_action_item_update",
      resource: `action_item:${id}`, changes: { status, assigneeId, deferReason: body.deferReason !== undefined, resolutionNote: body.resolutionNote !== undefined },
