...
//
// Mirror of the web module (artifacts/run-calculator/src/fillMissing.ts)
...
// paths (updateRunSettingsById / saveRecipePreset); there is no auto-apply.
...
// the app's own documented defaults, NOT the all-zero DEFAULT_SETTINGS blank-run
...
// Given a flat record of the run's current values (mobile: the RunSettings
// object; web: { ...formValues, brand, flavor }), return every applicable field
// that is still blank/zero.
...
// Mirrors the OpenAPI FillMissingInput contract (kept identical on web).
