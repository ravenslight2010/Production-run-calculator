...
function canonicalizeParsed(
  ...
  // its profile row after canonicalization renames. Mirrors web.
...
async function parseGridsCore(
  ...
  for (const chunk of chunks) {
    ...
    let ai = await requestParseSpecSheet({ workbookText, known: store.known, aliases });
  ...
  // per chunk — see extractEmbeddedApplicatorBlends. Mirrors web specImport.ts.
...
export async function commitSpecImport(
  ...
  // on app{n}OzPerPizza, not in the recipe name). Mirrors web (replit.md parity).
  ...
  // the pool can't be fetched, apply the imported names as-is. Mirrors web
  // (replit.md parity).
