...
export const syncConflictLogsTable = pgTable(
  'sync_conflict_logs',
  {
    ...
    clientStateHash: text('client_state_hash'), // SHA256 of client's form state
    serverStateHash: text('server_state_hash'), // SHA256 of server's persisted state
    mergedStateHash: text('merged_state_hash'), // SHA256 of result after merge
  ...
  (t) => ({
    scopeDateIdx: index('sync_conflict_logs_scope_date_idx').on(t.scope, t.date),
