...
export function tickWallClock(input: WallClockTickInput): { next: WallClockBookkeeping; events: WallClockTickEvent[] } {
  ...
  if (input.runStatus === "running" && input.calc.perTray > 0 && input.calc.ppm > 0) {
    ...
    else if (t.delta !== 0) { const to = Math.max(0, input.v.traysOnLine + t.delta); if (to !== input.v.traysOnLine) events.push({ channel: t.delta > 0 ? "tray-produce" : "tray-consume", dueAt: input.nowMs, nextDueAt: t.delta > 0 ? next.trayProdNextDueMs : next.trayConsNextDueMs, mutations: [{ field: "traysOnLine", from: input.form.traysOnLine || 0, to }] }); }
  }
  if (input.runStatus === "running" && input.calc.perBatch > 0 && input.calc.ppm > 0) {
    const b = computeBatchTick({ nowMs: input.nowMs, prodDueMs: next.batchProdNextDueMs, consDueMs: next.batchConsNextDueMs, lastMs: next.batchLastMs, periodMs: input.timing.batchConsumptionMs, fullBatchMs: input.timing.batchProductionMs, effDrainMs: Math.max(input.timing.hopperMs, input.calc.perBatch / input.calc.ppm * 60000), suppressed: input.doughSuppressed, feedComplete: input.calc.pressDone, deficitOpen: input.calc.batchesNeeded > 0, seeded: next.batchSeeded, current: input.form.batchesReady || 0, traysSeededAmount, traysNeeded: input.calc.traysNeeded, batchesNeeded: input.calc.batchesNeeded });
    ...
    else if (b.delta !== 0) { let to = input.v.batchesReady + b.delta; if (b.delta > 0) to = Math.min(to, Math.max(input.v.batchesReady, 3)); to = Math.max(0, Math.round(to * 100) / 100); if (to !== input.v.batchesReady) events.push({ channel: b.delta > 0 ? "batch-produce" : "batch-consume", dueAt: input.nowMs, nextDueAt: b.delta > 0 ? next.batchProdNextDueMs : next.batchConsNextDueMs, mutations: [{ field: "batchesReady", from: input.form.batchesReady || 0, to }] }); }
  }
  return { next, events };
}
