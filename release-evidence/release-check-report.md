...
Generated: 2026-08-26T00:30:17.232Z
Revision: 2e3b0bbbba40621d3d8859a201f0b77e5f32df82
Mode: full
...
| run calculator typecheck | PASS | 49s | `pnpm --filter @workspace/run-calculator run typecheck` |
| mockup sandbox typecheck | PASS | 8s | `pnpm --filter @workspace/mockup-sandbox run typecheck` |
| scripts typecheck | PASS | 2s | `pnpm --filter @workspace/scripts run typecheck` |
...
| API integration tests (release shard 2/6) | PASS | 95s | `pnpm --filter @workspace/api-server run test:release:integration:1` |
| API integration tests (release shard 3/6) | PASS | 68s | `pnpm --filter @workspace/api-server run test:release:integration:2` |
| API integration tests (release shard 4/6) | PASS | 176s | `pnpm --filter @workspace/api-server run test:release:integration:3` |
...
| run calculator tests | PASS | 217s | `pnpm --filter @workspace/run-calculator run test` |
...
| browser accessibility tests | PASS | 66s | `pnpm --filter @workspace/run-calculator run test:e2e:a11y` |
| full browser E2E suite | INFRASTRUCTURE TIMEOUT | 720s | `pnpm --filter @workspace/run-calculator run test:e2e` |
