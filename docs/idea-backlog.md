...
## 15. Expansion ideas (for when the app grows beyond its current scope)

These are bigger-picture ideas — not for now, but saved for when the app is
ready to expand into a full production operating system.

### Production intelligence
- **Downtime pattern correlation** — lightweight cause picker (2-3 taps) on
  pause; over weeks surfaces "Mondays, dough supply is 40% of downtime."
- **Predictive dough supply** — calculate next 3 days' dough batch needs from
  schedule + yield; push prep-time requirements to dough room night before.
  Pure math, no AI.
- **Yield variance heatmap** — actual vs theoretical pizzas-per-case over 30
  days per line/shift; surface drift from equipment wear.
- **Operator-initiated waste logging** — one-tap "waste event" (spoiled,
  mis-timed, damaged); builds first real waste database; HACCP teams would
  pay for this.

### Shop floor operations
- **Shift handoff card** — server generates structured "what incoming
  operator needs to know" (active runs, staged freezer, sauce barrel, dough
  supply status); push as SSE frame or PDF.
- **Prep phase countdown** — clear "prep checklist with countdown to line
  start" on phone/door screen so every station knows if behind schedule.
- **Ingredient expiry alerts on the line** — warn when a dough batch or
  ingredient nearing expiry is about to be used.

### Business analytics
- **Cost-per-run actual vs estimated** — live run cost vs target, variance
  % visible daily to the manager, not monthly from an accountant.
- **Line speed intelligence** — log actual ppm over shifts, correlate with
  equipment/staffing/weather; show realistic max and drift trends.
- **True daily cost vs menu price** — real margin per product from actual
  production data, not last year's estimates.

### Safety & compliance
- **HACCP temperature log integration** — manual or IoT sensor temperature
  capture centralized in the same app operators already use.
- **Equipment maintenance window** — track press/freezer/applicator last-
  service dates; surface upcoming maintenance on manager dashboard.

### Integration & ecosystem
- **Supplier API push** — auto-generate PO or send reorder message to
  supplier when dough supply projection shows a gap.
- **POS feedback loop** — compare planned production vs actual sales;
  flag over/under production per product for the next day.
- **Google Calendar/Shift export** — export scheduled runs + prep times to
  Google Calendar so all teams see production schedule in one place.

## 16. Quality control department expansion

QC already has: quality photo AI (pass/warn/fail), quality history, expiry/
waste insight, incidents (create/resolve/review + AI RCA + recurrence),
AI anomaly detection (deterministic drift + narration), downtime trends,
field checks, duplicate reviews, AI corrections, AI reviewer.

### Systematic quality monitoring
- **Quality trend dashboard** — pass-rate over time (line chart), by product
  type, by shift, by operator. Surface systemic drops (98% → 91% over a week).
- **Recurring issue clustering** — cluster similar quality check issues across
  days/shifts; surface "crust thickness variance flagged 12 times this week"
  instead of scattering across individual checks.
- **Quality score per run** — attach a quality score to each run so end-of-day
  summary shows quality alongside yield and timing. Makes quality part of the
  production record, not a separate silo.
- **Automated quality gates** — optional: "cannot end run until quality check
  passes" or "cannot start next run until previous quality review resolved."
  Configurable per shift.

### Inspection & compliance
- **Temperature logging** (manual entry now, IoT sensors future) — freezer/
  walk-in temps with timestamps; builds HACCP compliance record. Two fields,
  one tap, server stores with timestamps.
- **Allergen tracking per run** — surface allergens per run card based on
  recipes/ingredients. Critical for customer allergen queries and regulation.
- **Lot traceability** — capture lot numbers on ingredient intake, link to
  runs. If a customer reports an issue, trace back: which runs used which lots.
- **Supplier quality scoring** — correlate quality events with supply source;
  surface when a particular cheese supplier triggers 3x more quality warnings.

### Proactive quality intelligence
- **Predictive quality alerts** — warn *before* issues: dough temperature 2°F
  above spec, sauce batch at age limit. Data already tracked or cheap to add.
- **Quality spec drift alerts** — rolling averages of pizza weight, sauce oz,
  doughball weight vs recipe targets; flag drift over 5+ runs.
- **Cross-shift quality comparison** — surface shift-level differences (night
  98% vs day 91%) so management investigates staffing/equipment/procedures.
- **AI-powered quality root cause** — cross-reference failures against run
  context (operator, time, equipment state, freezer temp) to suggest root cause.

### Communication & accountability
- **Quality issue notification escalation** — push notify floor supervisor on
  critical quality issue; escalate to plant manager if not acknowledged in 15
  min. Uses existing web-push infra.
- **Quality review workflow** — failed checks go to manager review queue;
  "accept and correct" or "challenge with evidence." Same pattern as incident
  review workflow.
