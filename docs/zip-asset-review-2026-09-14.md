...
- **Retained evidence validation:** before attaching a copied or independently saved JSON capture to an audit record, run `python3 scripts/zip_asset_inventory.py --validate-report <approved-review-file>`. Validation accepts only the current redacted schema, bounded provenance, and the exact review-only label; legacy, malformed, or sensitive-looking reports fail closed.
