...
| Concurrent sync and inventory mutations | Disposable-Postgres integration tests for sync merge retries and inventory row-lock/idempotency boundaries | `@workspace/api-server run test:release:concurrency` | Required only when sync conflict/retry, inventory locking, consumption idempotency, or related transaction boundaries change; bounded to 180 seconds |
...
| Sync or inventory concurrency boundary | API typecheck; relevant route integration suite; `test:release:concurrency` (180-second budget) |
