...
Generated: 2026-08-27T10:27:28.846Z
Revision: 731c8addbf62a3a4dcf452853e8a4af5ea3616bf
Mode: full
...
| API server typecheck | PASS | 26s | `pnpm --filter @workspace/api-server run typecheck` |
...
| clean-start smoke | PASS | 39s | `pnpm run check:clean-start` |
...
| API integration tests (release shard 3/6) | INFRASTRUCTURE TIMEOUT | 480s | `pnpm --filter @workspace/api-server run test:release:integration:2` |
...
| API sync tests (release shard 5/6) | PASS | 60s | `pnpm --filter @workspace/api-server run test:release:sync` |
