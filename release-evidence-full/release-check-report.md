...
Generated: 2026-09-01T04:18:22.283Z
...
| run calculator typecheck | PASS | 39s | `pnpm --filter @workspace/run-calculator run typecheck` |
| mockup sandbox typecheck | PASS | 4s | `pnpm --filter @workspace/mockup-sandbox run typecheck` |
...
| API unit tests (release shard 1/6) | PASS | 41s | `pnpm --filter @workspace/api-server run test:release:unit` |
| API integration tests (release shard 2/6) | PASS | 146s | `pnpm --filter @workspace/api-server run test:release:integration:1` |
| API integration tests (release shard 3/6) | PASS | 102s | `pnpm --filter @workspace/api-server run test:release:integration:2` |
| API integration tests (release shard 4/6) | PASS | 386s | `pnpm --filter @workspace/api-server run test:release:integration:3` |
| API sync tests (release shard 5/6) | PASS | 36s | `pnpm --filter @workspace/api-server run test:release:sync` |
| API sync SSE tests (release shard 6/6) | PASS | 14s | `pnpm --filter @workspace/api-server run test:release:sync-sse` |
| run calculator tests | PASS | 196s | `pnpm --filter @workspace/run-calculator run test` |
...
| browser smoke tests | PASS | 31s | `pnpm --filter @workspace/run-calculator run test:e2e:smoke` |
| browser accessibility tests | PASS | 74s | `pnpm --filter @workspace/run-calculator run test:e2e:a11y` |
| full browser E2E suite | PASS | 1198s | `pnpm --filter @workspace/run-calculator run test:e2e` |
