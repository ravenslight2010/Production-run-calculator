...
Generated: 2026-08-28T01:35:00.229Z
Revision: 92d54045004c2679841a5b64abf6b9b0a68aa487
Mode: full
...
| run calculator typecheck | PASS | 48s | `pnpm --filter @workspace/run-calculator run typecheck` |
...
| API integration tests (release shard 2/6) | PASS | 201s | `pnpm --filter @workspace/api-server run test:release:integration:1` |
| API integration tests (release shard 3/6) | PASS | 140s | `pnpm --filter @workspace/api-server run test:release:integration:2` |
| API integration tests (release shard 4/6) | PASS | 374s | `pnpm --filter @workspace/api-server run test:release:integration:3` |
| API sync tests (release shard 5/6) | PASS | 18s | `pnpm --filter @workspace/api-server run test:release:sync` |
| API sync SSE tests (release shard 6/6) | PASS | 12s | `pnpm --filter @workspace/api-server run test:release:sync-sse` |
| run calculator tests | PASS | 210s | `pnpm --filter @workspace/run-calculator run test` |
...
| browser smoke tests | PASS | 49s | `pnpm --filter @workspace/run-calculator run test:e2e:smoke` |
| browser accessibility tests | PASS | 80s | `pnpm --filter @workspace/run-calculator run test:e2e:a11y` |
| full browser E2E suite | PASS | 1027s | `pnpm --filter @workspace/run-calculator run test:e2e` |
