...
- Drifted brand tags (pool spelling ≠ profile spelling, e.g. "Aldo" vs "Aldo's") shipped as heal `brand-drift-rename-v1` (2026-07-21): rewrites tags in all four pools + learns the brand aliases with chain re-point/flavor re-context via pure helpers in the api-server's brand-drift heal module. New drift pairs found later = new heal version with the pair added.
