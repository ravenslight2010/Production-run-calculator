...
Versioned complete day-state writes must identify the exact canonical snapshot they were based on. Validate that identity while holding the daily-row lock; on mismatch, return the complete canonical state without applying the incoming write so the client can rebase.
