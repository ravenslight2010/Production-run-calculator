...
**Rule:** Not just explicit endRun. Web `home.tsx` has TWO additional rollover paths (visibility/poll `checkDateRollover` and the midnight timer) that close active runs on a day change; both loop active runs and call `consumeRun`. Mobile `RunContext.tsx` has its day-rollover archive path (on load, when stored date != today) that calls `consumeRunInventory` per active run.
