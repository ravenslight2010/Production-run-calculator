...
- Prompt-only "emit every row" instructions are probabilistic: even with an explicit anti-subset rule (v10) the model once returned an "Extracted sample of yield rows" parse in prod (4 of 19 dough variants), which hash-reuse then pinned so retries couldn't heal it. v11 added explicit anti-sampling language; if under-extraction recurs, add a server-side quality guard (detect sample/subset notes or suspicious undercounts and retry/fail) instead of more prompt text.
