...
- **Thinking-token starvation:** Gemini "thinking" models draw thoughts from the SAME `maxOutputTokens` pool, so hidden thinking can consume the whole budget and return EMPTY visible text with `finishReason: MAX_TOKENS`. The adapter sets `thinkingConfig:{thinkingLevel: LOW}` on every call. **Do NOT use `thinkingBudget: 0`:** Gemini 3 models treat a zero budget as unsupported and silently fall back to DYNAMIC thinking on complex prompts — it worked on small synthetic prompts but the real `/ai/match-premix` prompt (with corrections + facility-memory blocks) burned ~3.9k thinking tokens of a 4096 budget and returned "" on every retry (route then quietly fell back to `{matches: []}` → user-visible import data loss). `thinkingLevel: LOW` is the supported knob for this model family; symptom to watch for: empty AI reply + ~30s latency + MAX_TOKENS.
...
- Models (main): both tiers currently = `gemini-3.6-flash` (see `src/models.ts`, the only source of truth). `gemini-2.5-flash` — briefly used on main — is retired: the direct Gemini API returns 404 "no longer available to new users" (verified 2026-09-22; Google points to `gemini-3.6-flash`). The Replit workstream branch historically pinned `gemini-3.1-pro-preview`; keep model names in sync with what the current deployment's provider actually serves. Preview/pro models add a harmless `thoughtSignature` on parts.
