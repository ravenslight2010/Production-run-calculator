...
- Convergence tests + induced patch-failure soak documented in `.agents/memory/`
...
**Server (temporary or permanent debug metrics):**
...
| Target | Rationale |
|--------|-----------|
| p95 PUT body **&lt; 500 KB** after delta or field trim | Comfortably under 10mb; restores headroom |
| Wake-to-fresh **&lt; 5 s** on facility Wi‑Fi | Floor usability |
| Queue depth **0** at documented handoff | No silent loss between shifts |

### 2.5 Decision

Treat **measurement as part of Phase A**, not a separate multi-week project. One PR that logs the server metrics above is enough to validate or revise the 500 KB target before or during delta-sync work.
...
FSMA 204 is strictest for foods on the Food Traceability List; a single-facility frozen pizza line may not need full KDE automation on day one, but **lot-level where-used** and **allergen control evidence** are still the industry baseline for serious buyers and many customers.
...
- Wake recovery diagnostics (Replit branch theme)  
...
| 2 | Instrument PUT/SSE sizes early; provisional p95 &lt; 500 KB | Optional `docs/sync-payload-baseline.md` once numbers exist |
...
- Link from [improvement-research-2026-09-18.md](improvement-research-2026-09-18.md) when that file lands on `main`.
