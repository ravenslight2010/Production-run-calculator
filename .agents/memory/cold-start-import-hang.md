...
**How to apply:** timeouts used — aliases GET 15s (fail-safe, importer proceeds without), AI parse 180s (legit 30-60s runtime), AI matchers + AI suggest-merges 120s (callers fall back to fuzzy/look-alike). The post-import background merge scan is covered too (its silent hang = "import finished but the duplicates toast never appeared"). Loading views also show a "server may be waking up" hint. If new import chains or other blocking-fetch dialogs are added, wire them through the same helper. A Reserved VM / min-instances deployment would remove cold starts entirely (user's cost call).

**Publishing gotcha:** the fix is client-side — an already-open tab keeps the OLD bundle until the user refreshes, so reports of "it still happens" right after publishing are expected. Publishing itself also restarts the deployment (a fresh cold start), so importing during/just after a publish reproduces the hang once on the old bundle. Also: name-check abort errors (`err.name === "TimeoutError"/"AbortError"`), never `instanceof DOMException` — cross-realm instanceof fails in tests/workers.
