...
- **Guarded direct-to-main delivery:** `pnpm run push:main -- --message "Describe the change"` validates with `pnpm run typecheck`, commits only explicitly staged changes, and pushes `origin/main`. It requires a clean unstaged worktree and local `main`, never stores credentials, and intentionally bypasses pull-request review. Set `push.main.requireSigned=true` in Git to require a verifiable commit signature before pushing; GitHub independently requires signed commits on `main` for every delivery path. Confirm that live rule with `pnpm run check:github-signed-commit-policy -- --repo OWNER/REPOSITORY`; see `docs/guarded-github-push.md` for accepted signing configuration and policy validation.
