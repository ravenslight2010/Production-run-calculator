...
- **Facility AI memory and incident context** — shared facts, corrections, and incident history used to ground AI features. Poisoning or disclosure can mislead operators or expose internal operational details.
...
- **Public → authenticated boundary** — `GET /api/healthz` and `/api/auth/*` are intentionally public; everything else is expected to require a valid session. Because production sign-up is public and not invite-gated, authenticated low-privilege access should be treated as internet-reachable during security review.
...
- **Highest-risk areas:** `src/routes/auth.ts`, `src/middlewares/requireAuth.ts`, `src/middlewares/requireCapability.ts`, `src/lib/auth.ts`, `src/lib/roles.ts`, `src/lib/sandbox.ts`, `src/routes/sync.ts`, `src/routes/inventory.ts`, `src/routes/ai.ts`, `src/routes/incidents.ts`
...
- **Authenticated/admin surface:** all other `/api/*` routes after router-level `requireAuth`; elevated routes are guarded per-handler with `requireCapability`. Since outsider self-registration is currently possible, routes that are only "authenticated" but not capability-gated should be reviewed as externally reachable.
...
Operators and managers can mutate schedules, inventory, production rules, aliases, and incident state. Because clients are untrusted, every server-side write path must validate inputs and enforce business rules and capabilities on the server, not in the client.
...
Managers and supervisors can assign roles, reset passwords, review incidents, and manage inventory. Any flaw in bootstrap logic, role assignment, sandbox identity, or route-level capability checks can turn a normal or anonymous user into a privileged one.

Required guarantees:
- Privileged roles must only be granted through deliberate, authorized flows.
- First-user/bootstrap behavior must not let an external attacker seize admin control in a fresh production deployment.
