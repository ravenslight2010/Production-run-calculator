Source: https://sre.google/workbook/monitoring/
Title: Google SRE - Monitoring Systems with Advanced Analytics
Fetched: 2026-09-19T16:07:42.888Z
...
01. [Table of Contents](https://sre.google/workbook/table-of-contents/)
02. [Foreword I](https://sre.google/workbook/foreword-I/)
03. [Foreword II](https://sre.google/workbook/foreword-II/)
04. [Preface](https://sre.google/workbook/preface/)
05. [1\. How SRE Relates to DevOps](https://sre.google/workbook/how-sre-relates/)
06. [Part I - Foundations](https://sre.google/workbook/part-I-foundations/)
07. [2\. Implementing SLOs](https://sre.google/workbook/implementing-slos/)
08. [3\. SLO Engineering Case Studies](https://sre.google/workbook/slo-engineering-case-studies/)
09. [4\. Monitoring](https://sre.google/workbook/monitoring/)
...
If you’re alerting based on metrics, it might be tempting to add more alerting based on logs—for example, if you need to be notified when even a single exceptional event happens. We still recommend metrics-based alerting in such cases: you can increment a counter metric when a particular event happens, and configure an alert based on that metric’s value. This strategy keeps all alert configuration in one place, making it easier to manage (see [Managing Your Monitoring System](https://sre.google/workbook/monitoring/#managing_your_monitoring_system)).
...
Ideally, you can instrument the lower-level RPC client library to export these metrics once, instead of asking each RPC client library to export them.[5](https://sre.google/workbook/monitoring/#ch04fn5) Instrumenting the client library provides more consistency and allows you to monitor new dependencies for free.
...
At Google, we test our monitoring and alerting using a domain-specific language that allows us to create synthetic time series. We then write assertions based upon the values in a [derived time series](https://sre.google/sre-book/practical-alerting/), or the firing status and label presence of specific alerts.

Monitoring and alerting is often a multistage process, which therefore calls for multiple families of unit tests. While this area remains largely underdeveloped, should you want to implement monitoring testing at some point, we recommend a three-tiered approach, as shown in [Figure 4-1](https://sre.google/workbook/monitoring/#monitoring-testing-environment-tiers).

![monitoring-testing-environment-tiers](https://lh3.googleusercontent.com/GuPDyFk1Ja635nOVF5H2iyb2ky15AhNLf_P174exWNgSvDB4BRnffIqYucibEdfHSgvq6-8sqe_BHbfEErvFD7hqniIBWseML33Kcw=s957)Figure 4-1. Monitoring testing environment tiers

1. _Binary reporting:_ Check that the exported metric variables change in value under certain conditions as expected.
2. _Monitoring configurations:_ Make sure that rule evaluation produces expected results, and that specific conditions produce the expected alerts.
3. _Alerting configurations:_ Test that generated alerts are routed to a predetermined destination, based on alert label values.
...
[1](https://sre.google/workbook/monitoring/#ch04fn1-marker)For example, using promtool to verify that your Prometheus config is syntactically correct.

[2](https://sre.google/workbook/monitoring/#ch04fn2-marker)You can export basic metrics via a common library: an instrumentation framework like [OpenCensus](https://opencensus.io/), or a service mesh like [Istio](https://istio.io/).

[3](https://sre.google/workbook/monitoring/#ch04fn3-marker)See [Chapter 10](https://sre.google/sre-book/practical-alerting/) of Site Reliability Engineering for Borgmon’s concepts and structure.

[4](https://sre.google/workbook/monitoring/#ch04fn4-marker)This is one case where monitoring via logs is appealing, particularly because production changes are relatively infrequent. Whether you use logs or metrics, these changes should be surfaced in your dashboards so they’re easily accessible for debugging production issues.

[5](https://sre.google/workbook/monitoring/#ch04fn5-marker)See [https://opencensus.io/](https://opencensus.io/) for a set of libraries that provides this.